      ...
      await bcrypt.compare(req.body.oldpass, user.dataValues.password, async function(err, result) {
        if(result == true){
          await bcrypt.hash(req.body.newpass, saltRounds, async function(err, hash) {
            ...
            transporter.sendMail(MailMessages.PasswordChanged, function(error, info){
              if (error) {
                console.log(error);
                res.json({
                  error: error,
      ...
      if(req.body){
        await bcrypt.hash(req.body.password, saltRounds, async function(err, hash) {
          ...
          await bcrypt.hash(token, saltRounds, async function(err, hashtok) {
            ...
            if(created) {
              transporter.sendMail(MailMessages.Activate, function(error, info){
                if (error) {
                  console.log(error);
                  res.json({
                    error: error,
...
app.post('/updateEmail', async (req, res) => {
  try {
    ...
    bcrypt.hash(token, saltRounds, async function (err, hash) {
      ...
      try {
        transporter.sendMail(MailMessages.EmailChange(req.body.newmail, user, token), function(error, info){
          if (error) {
            console.log(error);
            res.json({
              error: error,
