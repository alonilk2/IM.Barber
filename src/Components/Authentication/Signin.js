...
function SigninComponent (props) {
  ...
  const handleSubmit = event => {
    ...
    if (location.state && location.state.cartFlag === true)
      dispatch(
        signin(
          Email.toLowerCase(),
          password,
          location.state.cartFlag,
          location.state.cart,
          location.state.totalSum
        )
      )
      ...
      <div id='SignIncontainer'>
        <div className='row justify-content-center'>
          <div id='SignIn'>
            ...
            >
              {errorFromServer === 0 && (
                                <Alert severity="error" sx={{direction: 'rtl'}}>אימייל ו\או סיסמה אינם נכונים!</Alert>
